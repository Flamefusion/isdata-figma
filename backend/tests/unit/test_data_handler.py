...
from app.data_handler import (
    find_column,
    merge_ring_data_fast,
    load_sheet_data,
    load_sheets_data_parallel,
...
class TestMergeRingDataFast:
    ...
    def test_merge_with_valid_data(
        self, sample_step7_data, sample_vqc_data, sample_ft_data
    ):
        result, logs = merge_ring_data_fast(
...
class TestLoadSheetsDataParallel:
    ...
    def test_parallel_loading_success(self, mock_load_sheet):
        ...
        step7_data, vqc_data, ft_data, all_logs = load_sheets_data_parallel(
            config, mock_gc
        )
...
class TestSheetsConnection:
    ...
    def test_sheets_connection_invalid_service_account(self):
        config = {"serviceAccountContent": None, "vendorDataUrl": "https://test.url"}
